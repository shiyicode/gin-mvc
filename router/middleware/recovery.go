	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				...
				case error:
					resErr := errors.Warp(errors.ErrUnKnown, err.Error())
					c.JSON(controller.Failure(resErr))
					reqLog.Error(resErr.Error())
				default:
					resErr := errors.Warp(errors.ErrUnKnown)
					c.JSON(controller.Failure(resErr))

					reset := string([]byte{27, 91, 48, 109})
					stack := stack(3)
					httpRequest, _ := httputil.DumpRequest(c.Request, false)
					reqLog.Errorf("[Recovery] panic recovered:\n%s %s\n%s%s", string(httpRequest), err, string(stack), reset)
				}
			}
		}()
		c.Next()
